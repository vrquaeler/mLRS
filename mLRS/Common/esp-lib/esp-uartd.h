...
IRAM_ATTR uint16_t uartd_tx_notfull(void)
{
...
IRAM_ATTR void uartd_getbuf(char* buf, uint16_t len)
{
    UARTD_SERIAL_NO.readBytes(buf, len);
}
...
void _uartd_initit(uint32_t baud, UARTPARITYENUM parity, UARTSTOPBITENUM stopbits)
{
    ...
    UARTD_SERIAL_NO.setRxFIFOFull(64);  // > 57600 baud sets to 120 which is too much, buffer only 128 bytes
